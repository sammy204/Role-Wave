/* Expand the employer discovery response for opted-in RoleWave Pro candidates. */

drop function if exists public.employer_discover_candidates(text);

create function public.employer_discover_candidates(p_search text default null)
returns table (
  ...
  open_to_work boolean, updated_at timestamptz
)
...
begin
  if not exists (
    ...
    where p.id = auth.uid() and p.account_type = 'employer'
      and c.owner_profile_id = auth.uid() and c.verified = true
  ) then
    raise exception 'Only verified employer accounts can discover candidates.';
  end if;

  return query
  select cp.id, p.full_name, cp.avatar_url, cp.headline, cp.bio, cp.location,
    cp.years_experience, cp.skills, cp.preferred_locations, cp.preferred_job_titles,
    cp.preferred_salary, cp.job_type, cp.work_preference, cp.availability,
    cp.resume_url, cp.resume_name, cp.work_authorization, cp.portfolio_url,
    cp.github_url, cp.linkedin_url, cp.education, cp.experience, cp.projects,
    cp.open_to_work, cp.updated_at
  ...
  where cp.visibility_to_employers in ('open', 'not_open')
    and p.account_type = 'candidate'
    and entitlement.product = 'ai_features' and entitlement.status = 'active'
    and (entitlement.current_period_end is null or entitlement.current_period_end > now())
    and (search_term is null or cp.headline ilike '%' || search_term || '%'
      or cp.bio ilike '%' || search_term || '%' or cp.location ilike '%' || search_term || '%'
  ...
  and exists (
    ...
    where (storage.foldername(name))[1] = cp.id::text
      ...
      and exists (
        ...
        join public.companies c on c.id = ep.company_id
        where ep.id = auth.uid() and c.owner_profile_id = auth.uid() and c.verified = true
      )
  )
);
