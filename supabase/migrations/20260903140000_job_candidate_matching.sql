/* Candidate discovery is always scoped to one employer-owned job. */

drop function if exists public.employer_discover_candidates(text);
drop function if exists public.employer_discover_candidates(uuid, text);

create function public.employer_discover_candidates(p_job_id uuid, p_search text default null)
returns table (
...
begin
  if not exists (
    ...
    where j.id = p_job_id and c.owner_profile_id = auth.uid()
      and c.verified = true and p.account_type = 'employer'
  ) then
  ...
  with job_context as (
    ...
    select cp.*, p.full_name, cp.avatar_url,
      (
    ...
    join public.ai_entitlements e on e.user_id = cp.id
    cross join job_context j
    where cp.visibility_to_employers in ('open', 'not_open')
      and p.account_type = 'candidate' and e.product = 'ai_features' and e.status = 'active'
      and (e.current_period_end is null or e.current_period_end > now())
      and (
        exists (select 1 from unnest(cp.skills) skill, unnest(j.tags) tag where lower(skill) = lower(tag))
  ...
  select s.id, s.full_name, s.avatar_url, s.headline, s.bio, s.location, s.years_experience,
    s.skills, s.preferred_locations, s.preferred_job_titles, s.preferred_salary, s.job_type,
    s.work_preference, s.availability, s.resume_url, s.resume_name, s.work_authorization,
  ...
  from scored s
  where search_term is null or s.full_name ilike '%' || search_term || '%'
...
revoke all on function public.employer_discover_candidates(uuid, text) from public;
grant execute on function public.employer_discover_candidates(uuid, text) to authenticated;
